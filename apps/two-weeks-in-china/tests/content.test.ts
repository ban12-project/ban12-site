...
import {
  buildMenu, canonicalJson, checksum, createManifest, pagesByPrefix, parsePage,
  publishedPages, readCatalog, serializePage, validateCountries, validateFrontmatter,
  validatePath, validateSnapshot, verifySnapshot,
} from '../lib/content/core';
import type { ContentSnapshot, Page, PageFrontmatter } from '../lib/content/types';
...
function frontmatter(overrides: Partial<PageFrontmatter> = {}): PageFrontmatter {
  return {
    id: 1, path: '/en/plan-trip/fixture', title: 'Synthetic test article', subtitle: null,
    metadata: { order: 3, icon: 'plane', tags: ['test'], extra: { preserved: true } },
    createdAt: '2025-01-01T12:00:00', updatedAt: '2025-01-02T12:00:00.123456', draft: false,
    ...overrides,
  };
}
function page(overrides: Partial<Page> = {}): Page {
  return { ...frontmatter(), content: 'Synthetic content\n', format: 'md', ...overrides };
}
function snapshot(): ContentSnapshot {
  const { draft: _draft, ...fields } = frontmatter();
  return {
    version: 1, exportedAt: '2026-09-06T01:00:00Z',
    pages: [{ ...fields, id: 1, content: '\n<InfoBox title="Fixture">\r\n\n**正文** 🌏\r\n\n</InfoBox>\n' }],
    countries: [{
      id: 1, name: 'Synthetic fixture country', code: 'ZZ', visaPolicy: 'visa_required',
      policyDetails: null, createdAt: null, updatedAt: null,
    }],
  };
}
function imported() {
...
test('frontmatter and body round-trip without trimming or newline changes', () => {
  ...
  const result = parsePage(serializePage(frontmatter(), body), 'en/plan-trip/fixture.mdx');
...
test('plain Markdown has md format and keeps braces as content', () => {
  assert.equal(parsePage(serializePage(frontmatter(), 'Use {braces} and <literal>'), 'en/plan-trip/fixture.md').format, 'md');
});
test('CRLF frontmatter is supported without rewriting CRLF body', () => {
  const source = `---\r\n${JSON.stringify(frontmatter(), null, 2)}\r\n---\r\nbody\r\n`;
  assert.equal(parsePage(source, 'en/plan-trip/fixture.md').content, 'body\r\n');
});
test('invalid JSON frontmatter fails instead of evaluating JavaScript', () => {
  assert.throws(() => parsePage('---\n{draft: false}\n---\nbody', 'en/plan-trip/fixture.md'), /Invalid JSON/);
});
test('missing frontmatter and unsupported extension fail', () => {
  assert.throws(() => parsePage('body', 'en/plan-trip/fixture.md'), /Missing JSON/);
  assert.throws(() => parsePage('body', 'en/plan-trip/fixture.txt'), /Unsupported content file/);
});
test('filename must match canonical path and locale', () => {
  assert.throws(() => parsePage(serializePage(frontmatter(), 'body'), 'zh/plan-trip/fixture.md'), /Path\/file mismatch/);
});
test('draft flag is mandatory and typed', () => {
  assert.throws(() => validateFrontmatter({ ...frontmatter(), draft: undefined }), /explicitly/);
  assert.throws(() => validateFrontmatter({ ...frontmatter(), draft: 'false' }), /explicitly/);
});
test('unknown frontmatter fields fail rather than silently disappearing', () => {
  assert.throws(() => validateFrontmatter({ ...frontmatter(), published: true }), /Unknown frontmatter/);
});
test('published empty body fails while an empty draft is permitted', () => {
  assert.throws(() => parsePage(serializePage(frontmatter(), ' \n'), 'en/plan-trip/fixture.md'), /empty/);
  assert.equal(parsePage(serializePage(frontmatter({ draft: true }), ''), 'en/plan-trip/fixture.md').content, '');
});
test('unsafe, unsupported and conflicting paths fail', () => {
  for (const path of ['/en/../secret', '/en/%2e%2e/secret', '/en//file', '/en/file/', '/en/a?b',
    '/en/a#b', '/en/a b', '/en/a\\b', '/fr/a', '/en', '/en/eligibility', '/en/eligibility/child']) {
...
test('metadata is validated, including tag arrays and finite order', () => {
  assert.throws(() => validateFrontmatter(frontmatter({ metadata: { order: Number.NaN } })), /finite/);
  assert.throws(() => validateFrontmatter({ ...frontmatter(), metadata: { tags: [3] } }), /tags/);
});
test('nullable legacy fields and two/three-letter country codes survive', () => {
  const country = snapshot().countries[0];
  for (const code of [null, 'ZZ', 'ZZZ']) assert.equal(validateCountries([{ ...country, code }])[0].code, code);
...
test('duplicate country names and IDs fail', () => {
  const country = snapshot().countries[0];
  assert.throws(() => validateCountries([country, { ...country, id: 2 }]), /Duplicate/);
  assert.throws(() => validateCountries([country, { ...country, name: 'Other fixture' }]), /Duplicate/);
});
test('empty countries, unknown policies and unknown fields fail', () => {
  assert.throws(() => validateCountries([]), /empty placeholder/);
  assert.throws(() => validateCountries([{ ...snapshot().countries[0], visaPolicy: 'invented' }]), /Unknown visa policy/);
  assert.throws(() => validateCountries([{ ...snapshot().countries[0], extra: 1 }]), /Unknown country field/);
});
test('snapshot rejects empty tables and duplicate article IDs or paths', () => {
  const data = snapshot();
  assert.throws(() => validateSnapshot({ ...data, pages: [] }), /no pages/);
  assert.throws(() => validateSnapshot({ ...data, countries: [] }), /empty placeholder/);
  assert.throws(() => validateSnapshot({ ...data, pages: [data.pages[0], { ...data.pages[0], id: 2 }] }), /Duplicate/);
  assert.throws(() => validateSnapshot({ ...data, pages: [data.pages[0], { ...data.pages[0], path: '/en/other' }] }), /Duplicate/);
});
...
test('drafts are absent from published pages and section navigation', () => {
  const pages = [page(), page({ id: 2, path: '/en/plan-trip/draft', draft: true })];
...
test('navigation has stable numeric order then path order', () => {
  const pages = [page({ id: 2, path: '/en/plan-trip/z', metadata: { order: 1 } }),
    page({ id: 3, path: '/en/plan-trip/a', metadata: { order: 1 } }), page()];
  assert.deepEqual(buildMenu(pages, 'en')[0].children.map((child) => child.href),
...
test('missing or awaiting-export manifest blocks reads before empty content can deploy', () => {
  ...
  writeFileSync(join(root, 'migration.json'), JSON.stringify({ version: 1, status: 'awaiting-export' }));
...
test('import keeps content docs and templates intact', () => {
  ...
  assert.equal(readFileSync(join(root, 'templates/article.md'), 'utf8'), 'retained template');
});
test('identical reimport is idempotent and a changed snapshot is refused', () => {
  const { root, data } = imported();
  importSnapshot(data, root);
  const changed = structuredClone(data); changed.pages[0].content += 'changed';
...
test('new drafts are allowed without changing the migration baseline', () => {
  const { root } = imported();
  writeFileSync(join(root, 'pages/en/plan-trip/draft.md'),
    serializePage(frontmatter({ id: undefined, path: '/en/plan-trip/draft', draft: true }), ''));
...
test('invalid import does not partially write pages or a completion manifest', () => {
  const root = directory();
  const data = snapshot(); data.pages[0].path = '/en/../escape';
...
test('tampered country values fail initial acceptance check', () => {
  const { root, data } = imported();
  writeFileSync(join(root, 'countries.json'), JSON.stringify([{ ...data.countries[0], policyDetails: 'changed' }]));
  assert.throws(() => verifySnapshot(readCatalog(root), validateSnapshot(data)), /Countries differ/);
});
test('tampered manifest counts fail initial acceptance check', () => {
  const { root, data } = imported();
  writeFileSync(join(root, 'migration.json'), JSON.stringify({ ...createManifest(data), countryCount: 3 }));
  assert.throws(() => verifySnapshot(readCatalog(root), validateSnapshot(data)), /manifest count/);
});
...
`, { mode: 0o700 });
  ...
  const env = { PATH: root, CONTENT_DATABASE_URL: 'postgresql://fixture:synthetic-password@db.example.invalid/fixture',
    PGSERVICE: 'wrong-database', PGHOSTADDR: '192.0.2.1', DATABASE_URL: 'unrelated-placeholder' };
  const run = () => spawnSync(process.execPath, [join(__dirname, '../scripts/content/export.js'), output],
    { env, encoding: 'utf8' });
...
test('exporter uses read-only SQL, verified TLS, UTF-8 and no credentials in arguments', () => {
  ...
  assert.equal((result.stdout + result.stderr).includes('synthetic-password'), false);
  assert.deepEqual(JSON.parse(readFileSync(setup.output, 'utf8')), validateSnapshot(setup.data));
});
...
test('exporter with no credential fails before invoking psql', () => {
  const setup = mockExport();
  const result = spawnSync(process.execPath, [join(__dirname, '../scripts/content/export.js'), setup.output],
