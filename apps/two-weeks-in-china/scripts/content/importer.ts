...
import { createManifest, readCatalog, serializePage, validateSnapshot, verifySnapshot } from '../../lib/content/core';
...
export function importSnapshot(value: unknown, destination: string): ContentSnapshot {
  const snapshot = validateSnapshot(value);
  const target = resolve(destination);
  const parent = dirname(target);
  mkdirSync(parent, { recursive: true });
  if (existsSync(target)) {
    if (hasSymlinks(target)) throw new Error('Refusing to replace a symlinked content tree');
    const pages = join(target, 'pages');
    const countries = join(target, 'countries.json');
    if (existsSync(countries) || (existsSync(pages) && readdirSync(pages).length > 0)) {
  ...
  try {
    ...
    for (const page of snapshot.pages) {
      ...
      writeFileSync(file, serializePage({ ...frontmatter, draft: false }, content), { flag: 'wx' });
    }
    writeFileSync(join(staging, 'countries.json'), `${JSON.stringify(snapshot.countries, null, 2)}\n`);
    writeFileSync(join(staging, 'migration.json'), `${JSON.stringify(createManifest(snapshot), null, 2)}\n`);
    verifySnapshot(readCatalog(staging), snapshot);
    if (existsSync(target)) { renameSync(target, backup); movedPrevious = true; }
    try { renameSync(staging, target); installed = true; }
    catch (error) {
    ...
    if (movedPrevious && !installed) {
      console.error(`Import rollback needs attention; preserved backup: ${backup}`);
