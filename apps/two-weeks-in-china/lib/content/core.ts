...
function timestamp(value: unknown, label: string): string {
  ...
  invariant(
    /^\d{4}-\d{2}-\d{2}[T ][\d:.]+(?:Z|[+-]\d{2}(?::?\d{2})?)?$/.test(result) &&
...
export function validatePath(value: unknown): string {
  ...
  invariant(
    ...
    `Unsafe content path: ${path}`,
  );
  const parts = path.slice(1).split('/');
  invariant(
    parts.length >= 2 && LOCALES.has(parts[0]),
    `Unsupported locale or empty slug: ${path}`,
  );
  invariant(
  ...
  invariant(
    parts[1] !== 'eligibility',
    `Content path conflicts with the eligibility route: ${path}`,
  );
  return path;
}
...
function metadata(value: unknown): PageMetadata | null {
  ...
  if (result.order !== undefined) {
    invariant(
      typeof result.order === 'number' && Number.isFinite(result.order),
      'metadata.order must be finite',
    );
  }
...
export function validateFrontmatter(value: unknown): PageFrontmatter {
  ...
  for (const key of Object.keys(row))
    invariant(allowed.has(key), `Unknown frontmatter field: ${key}`);
  invariant(
    typeof row.draft === 'boolean',
    'frontmatter.draft must be explicitly true or false',
  );
  const result: PageFrontmatter = {
    path: validatePath(row.path),
    title: text(row.title, 'title'),
    subtitle:
      row.subtitle === undefined
        ? null
        : nullableText(row.subtitle, 'subtitle'),
...
export function validateCountries(value: unknown): Country[] {
  invariant(
    ...
    'countries.json must contain the exported country rows, not an empty placeholder',
  );
  ...
  return value
    .map((item) => {
      ...
      for (const key of Object.keys(row))
        invariant(allowed.has(key), `Unknown country field: ${key}`);
      const id = integer(row.id, 'country.id');
      const name = text(row.name, 'country.name');
      invariant(
        !names.has(name) && !ids.has(id),
        `Duplicate country name/id: ${name}`,
      );
      names.add(name);
      ids.add(id);
      ...
      invariant(
        code === null || /^[A-Za-z]{2,3}$/.test(code),
        `Invalid country code: ${name}`,
      );
      invariant(
        VISA_POLICIES.some((policy) => policy === row.visaPolicy),
        `Unknown visa policy: ${name}`,
      );
      return {
        id,
        name,
        code,
        visaPolicy: row.visaPolicy as Country['visaPolicy'],
        policyDetails: nullableText(row.policyDetails, 'country.policyDetails'),
        createdAt:
          row.createdAt === null
            ? null
            : timestamp(row.createdAt, 'country.createdAt'),
        updatedAt:
          row.updatedAt === null
            ? null
            : timestamp(row.updatedAt, 'country.updatedAt'),
      };
    })
...
export function parsePage(source: string, filename: string): Page {
  ...
  invariant(
    ...
    `Unsupported content file: ${filename}`,
  );
  const match = /^(?:\uFEFF)?---\r?\n([\s\S]*?)\r?\n---\r?\n/.exec(source);
  ...
  try {
    ...
    throw new Error(`Invalid JSON frontmatter: ${filename}`);
  }
  const frontmatter = validateFrontmatter(value);
  const expected = `${frontmatter.path.slice(1)}.${format}`;
  invariant(
    filename.split(sep).join('/') === expected,
    `Path/file mismatch: ${filename} must be ${expected}`,
  );
  const content = source.slice(match[0].length);
  invariant(
    frontmatter.draft || content.trim().length > 0,
    `Published content is empty: ${filename}`,
  );
...
function filesUnder(directory: string): string[] {
  ...
  invariant(
    ...
    `Symlink is not allowed in content: ${directory}`,
  );
  return readdirSync(directory)
    ...
    .flatMap((name) => {
      ...
      invariant(
        !stat.isSymbolicLink(),
        `Symlink is not allowed in content: ${file}`,
      );
      if (stat.isDirectory()) return filesUnder(file);
      invariant(
        stat.isFile() && /\.(md|mdx)$/.test(name),
        `Unexpected file in content/pages: ${file}`,
      );
      return [file];
    });
}
...
function manifest(value: unknown): MigrationManifest {
  ...
  invariant(
    data.version === 1 && data.status === 'complete',
  ...
  invariant(
    SHA256.test(text(data.sourceSha256, 'migration.sourceSha256')),
    'Invalid source checksum',
  );
  integer(data.pageCount, 'migration.pageCount');
  integer(data.countryCount, 'migration.countryCount');
  invariant(
    Array.isArray(data.pages) && data.pages.length === data.pageCount,
    'Invalid migration page count',
  );
  const paths = new Set<string>();
  for (const entry of data.pages) {
    const page = record(entry, 'migration page');
    const path = validatePath(page.path);
    invariant(!paths.has(path), `Duplicate migration path: ${path}`);
    paths.add(path);
    invariant(
      SHA256.test(text(page.sha256, 'migration page sha256')),
      'Invalid page checksum',
    );
  }
  invariant(
    SHA256.test(text(data.countriesSha256, 'migration.countriesSha256')),
    'Invalid country checksum',
  );
...
export function readCatalog(directory: string): Catalog {
  ...
  invariant(
    existsSync(root) && !lstatSync(root).isSymbolicLink(),
    'Missing or symlinked content directory',
  );
  invariant(
    existsSync(join(root, 'migration.json')),
    'Missing content/migration.json: import the original content first',
  );
  const migration = manifest(readJson(join(root, 'migration.json')));
  const pagesRoot = join(root, 'pages');
  const pages = filesUnder(pagesRoot).map((file) =>
    parsePage(readFileSync(file, 'utf8'), relative(pagesRoot, file)),
  );
  const paths = new Set<string>();
  const ids = new Set<number>();
  const filenames = new Set<string>();
  for (const page of pages) {
    invariant(!paths.has(page.path), `Duplicate content path: ${page.path}`);
    paths.add(page.path);
    // Also catch case-only filename collisions on macOS/Windows.
    invariant(
      !filenames.has(page.path.toLowerCase()),
      `Case-colliding content path: ${page.path}`,
    );
    filenames.add(page.path.toLowerCase());
    if (page.id !== undefined) {
      invariant(!ids.has(page.id), `Duplicate legacy page id: ${page.id}`);
      ids.add(page.id);
    }
  }
  invariant(
    pages.length > 0,
    'No article files found: migration must not publish an empty site',
  );
  ...
  for (const baseline of migration.pages) {
    invariant(
      pages.some((page) => page.path === baseline.path && !page.draft),
      `Missing published legacy path: ${baseline.path}`,
    );
  }
  const countries = validateCountries(readJson(join(root, 'countries.json')));
  return {
    pages: pages.sort((a, b) => a.path.localeCompare(b.path, 'en')),
    countries,
    migration,
  };
}
...
export function validateSnapshot(value: unknown): ContentSnapshot {
  ...
  invariant(
    ...
    'The export has no pages; refusing an empty migration',
  );
  const paths = new Set<string>();
  const ids = new Set<number>();
  const pages = data.pages
    .map((item) => {
      ...
      invariant(
        !paths.has(frontmatter.path) && !ids.has(id),
        `Duplicate exported page: ${frontmatter.path}`,
      );
      paths.add(frontmatter.path);
      ids.add(id);
  ...
  return {
    ...
    exportedAt,
    pages,
    countries: validateCountries(data.countries),
  };
}
...
export function createManifest(snapshot: ContentSnapshot): MigrationManifest {
  return {
    ...
    sourceSha256: checksum(snapshot),
    pageCount: snapshot.pages.length,
    countryCount: snapshot.countries.length,
    pages: snapshot.pages.map((page) => ({
      path: page.path,
      sha256: checksum(page),
    })),
...
export function verifySnapshot(
  ...
  invariant(
    catalog.migration.sourceSha256 === checksum(snapshot),
    'Export does not match the migration manifest',
  );
  invariant(
    catalog.migration.pageCount === snapshot.pages.length &&
      catalog.migration.countryCount === snapshot.countries.length,
    'Migration manifest count differs from the export',
  );
  invariant(
    catalog.pages.length === snapshot.pages.length,
    'Article count differs from the original export',
  );
  invariant(
    catalog.countries.length === snapshot.countries.length,
    'Country count differs from the original export',
  );
  for (const original of snapshot.pages) {
    ...
    invariant(
      checksum(stored) === checksum(original),
      `Article data differs from the export: ${original.path}`,
    );
    invariant(
      catalog.migration.pages.some(
        (item) =>
          item.path === original.path && item.sha256 === checksum(original),
      ),
      `Article manifest differs from the export: ${original.path}`,
    );
  }
  invariant(
    checksum(catalog.countries) === checksum(snapshot.countries),
    'Countries differ from the original export',
  );
  invariant(
    catalog.migration.countriesSha256 === checksum(snapshot.countries),
    'Country manifest differs from the export',
  );
}
...
export function pagesByPrefix(pages: Page[], prefix: string): Page[] {
  ...
  return publishedPages(pages).filter(
    (page) =>
      page.path === normalized || page.path.startsWith(`${normalized}/`),
  );
}
