...
const qrcode: QRCodeFactory = (() => {
  ...
  const gifImage = (width, height) => {
    ...
    const lzwTable = () => {
      ...
      _this.indexOf = (key) => _map[key];

      _this.contains = (key) => typeof _map[key] !== 'undefined';
  ...
  const createDataURL = (width, height, getPixel) => {
