...
export function CommandMenu(props: Props) {
  ...
  return (
    <>
      ...
      <Button
        variant="outline"
        className="bg-muted/80 text-muted-foreground hidden h-8 w-full justify-start rounded-xl text-sm font-normal shadow-none saturate-180 backdrop-blur-[20px] backdrop-filter sm:pr-12 md:inline-flex md:w-40 lg:w-56 xl:w-64"
        ...
        {data.map((restaurant) => (
          <CommandItem
            ...
            onSelect={() => {
              ...
              navigation(
                ...coordtransform.gcj02towgs84(...restaurant.location),
              );
            }}
          >
    ...
    <Drawer.Root {...props}>
      <Drawer.Portal>
        ...
        <Drawer.Content className="fixed bottom-0 left-0 right-0 mt-24 flex h-[80vh] flex-col rounded-t-[10px] outline-none">
          ...
          <Command className="**:[[cmdk-group-heading]]:text-muted-foreground **:data-[slot=command-input-wrapper]:h-12 *:[[cmdk-list]]:pb-safe-max-4 rounded-none *:[[cmdk-list]]:max-h-none **:[[cmdk-group-heading]]:px-2 **:[[cmdk-group-heading]]:font-medium [&_[cmdk-group]:not([hidden])_~[cmdk-group]]:pt-0 **:[[cmdk-group]]:px-2 [&_[cmdk-input-wrapper]_svg]:h-5 [&_[cmdk-input-wrapper]_svg]:w-5 **:[[cmdk-input]]:h-12 **:[[cmdk-item]]:px-2 **:[[cmdk-item]]:py-3 [&_[cmdk-item]_svg]:h-5 [&_[cmdk-item]_svg]:w-5">
            {children}
          </Command>
        </Drawer.Content>
      </Drawer.Portal>
