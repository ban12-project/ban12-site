...
function VoxelMesh({ voxelsPerModel }: { voxelsPerModel: Voxel[] }) {
  ...
  const randomCoordinate = React.useCallback(() => {
  ...
  React.useEffect(() => {
    ...
    setVoxels(voxels);
  }, [dummy, voxelsPerModel, randomCoordinate]);
  ...
  useGSAP(() => {
    ...
    for (let i = 0; i < voxels.length; i++) {
      const duration = 0.5 + 0.5 * Math.random() ** 6;
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: mesh
    <instancedMesh
