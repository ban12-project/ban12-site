  ...
  return (
    <section className="hidden-scrollbar px-safe-max-4 flex snap-x snap-mandatory gap-x-3 overflow-x-auto overscroll-x-contain lg:gap-x-5 lg:px-0 lg:pb-10">
      {collections.map((item, index) => (
        <div
          className="box-content w-full shrink-0 snap-center pb-10 pt-2.5 md:w-100 lg:snap-start lg:last:ltr:pe-[calc(var(--container-inset,0)*2)] lg:first:rtl:ps-[calc(var(--container-inset,0)*2)]"
          key={item.id}
        >
          <SuperEllipse
            asChild
            svgProps={{ width: WIDTH, height: HEIGHT, n: 20, steps: 512 }}
          >
            <Link
            ...
            >
              <div
                className="absolute p-7 text-(--text-color,#fff) lg:p-7.5"
                style={
                  { '--text-color': item.textColor } as React.CSSProperties
                }
              >
                <h2 className="text-2xl font-bold">{item.title[lang]}</h2>
              </div>
              <Image
